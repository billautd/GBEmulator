...
TEST_CLASS(OperationsTests_ld_r16mem_a) {
public:
	...
	BEGIN_TEST_METHOD_ATTRIBUTE(OperationsTests_ld_bc_a_0x02)
		TEST_DESCRIPTION("Test on LD [BC],A instruction for code 0x02")
	END_TEST_METHOD_ATTRIBUTE()
	TEST_METHOD(OperationsTests_ld_bc_a_0x02) {
		Registers& regs = Context::get()->getRegisters();

		Context::get()->setCurrentOpCode(0x02);
		//Value to copy
		regs.a = 0xEF;
		//Byte pointed to
		regs.b = 0xAB;
		...
		Operations::ld_r16mem_a();
		//Check registers not changed");
		Assert::AreEqual((u8)0xEF, regs.a);
		Assert::AreEqual((u8)0xAB, regs.b);
		Assert::AreEqual((u8)0xCD, regs.c);
		...
		//Check value changed at byte pointed to");
		Assert::AreEqual((u8)0xEF, (u8)Context::get()->getMemory().at(0xABCD));
