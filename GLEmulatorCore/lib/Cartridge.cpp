...
void Cartridge::readBytesFromRomData(size_t offset, const std::span<u8>& array) {
	std::copy_n(Context::get()->getMemory().begin() + offset, array.size(), array.begin());
}

bool Cartridge::checkHeaderChecksum() {
	u8 checksum = 0;
	for (u16 address = 0x0134; address <= 0x14C; address++) {
		checksum = checksum - Context::get()->getMemory().at(address) - 1;
	}
...
bool Cartridge::checkGlobalChecksum() {
	...
	for (size_t i = 0; i < actualRomSize; i++) {
		...
		checksum += Context::get()->getMemory().at(i);
	}
	//Big endian
...
void Cartridge::loadCartrige(const char* path)
{
	...
	file.seekg(0);
	file.read(reinterpret_cast<char*>(Context::get()->getMemory().data()), actualRomSize);
